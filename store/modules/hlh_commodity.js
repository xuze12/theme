...
export default {
  ...
  actions: {
    ...
    async getPageProdsList({ commit, dispatch }, payload) {
      ...
      dispatch('getCommonList', { method: getPageProds, params: { categoryId: payload } })
    },
    ...
    async getProdInfo(context, payload) {
      ...
      const is_true = !(data && typeof data == "object" && Reflect.has(data, 'prodId'))
      if (is_true) {
    ...
    changeSkuId(context, payload) {
      context.commit("save", { skuId: payload.skuId, totalStocks: payload.stocks === -1 ? 99999 : payload.stocks });
    },
    // 改变库存值
    ...
    async getProdByTagId({ commit, state, dispatch }, payload) {
      dispatch('getCommonList', { method: getProdListByTagId, params: { tagId: state.tagId } })
    },
    ...
    async pageSizeChange({ dispatch, commit, state }, payload) {
      ...
      if (state.categoryId) {
        dispatch('getCommonList', { method: getPageProds, params: { categoryId: state.categoryId } })
        return
      }
